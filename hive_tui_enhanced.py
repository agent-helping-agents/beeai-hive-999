...
class TypingIndicator:
    ...
    def get_current_frame(self) -> str:
        ...
        frame_index = int(elapsed * 3) % len(self.animation_frames)
...
class HiveEnhancedTUI:
    ...
    def __init__(self):
        ...
        self.current_model: str = "marco-o1:latest"
        self.mode: str = "queen"
                ...
                with open(self.history_file, "r") as f:
                    ...
                    for msg_data in history_data:
                        msg = Message(
                            ...
                            agent_type=AgentType[msg_data["agent_type"]],
                            is_code=msg_data.get("is_code", False),
                            code_language=msg_data.get("code_language"),
                            is_thinking=msg_data.get("is_thinking", False),
                        )
                ...
                history_data.append(
                    {
                        ...
                        "agent_type": msg.agent_type.name,
                        "is_code": msg.is_code,
    ...
    def add_message(self, message: Message):
        ...
        # Detect code blocks and set language
        if "```" in message.content:
            message.is_code = True
            # Extract code language from ```lang format
            import re

            lang_match = re.search(r"```(\w+)", message.content)
            if lang_match:
                message.code_language = lang_match.group(1)

        self.messages.append(message)
        if len(self.messages) > self.max_chat_history:
            self.messages = self.messages[-self.max_chat_history :]
        self.save_message_history()

    def format_message_for_display(self, message: Message) -> AnyFormattedText:
        """Format message with rich text, syntax highlighting, and hexagonal borders."""
        # Base formatting
        if message.is_code:
            return self._format_code_message(message)
        elif message.is_thinking:
            return self._format_thinking_message(message)
        else:
            return self._format_regular_message(message)

    def _format_code_message(self, message: Message) -> AnyFormattedText:
        """Format code messages with syntax highlighting and hexagonal borders."""
        try:
            ...
            for i, part in enumerate(code_blocks):
                if i % 2 == 0:  # Regular text
                    if part.strip():
                        formatted_parts.append(HTML(f"<dim>{part}</dim>"))
                else:  # Code block
                    # Split language from code
                    lines = part.split("\n", 1)
                    ...
                    try:
                        ...
                        # Highlight code
                        highlighted = highlight(code, lexer, Terminal256Formatter())
                        formatted_parts.append(HTML(f"<pre>{highlighted}</pre>"))

                    except Exception as e:
                        # Fallback to plain text
                        formatted_parts.append(HTML(f"<pre>{code}</pre>"))

            return merge_formatted_text(formatted_parts)

        except ImportError:
            # If pygments not available, just show as preformatted
            return HTML(f"<pre>{message.content}</pre>")
        except Exception as e:
            return HTML(f"<error>Error formatting code: {e}</error>\n{message.content}")

    def _format_thinking_message(self, message: Message) -> AnyFormattedText:
        """Format thinking messages with animated indicators."""
        return HTML(f"<dim>🤔 {message.sender} is thinking...</dim>\n{message.content}")

    def _format_regular_message(self, message: Message) -> AnyFormattedText:
        """Format regular messages with agent-specific colors."""
        # Get agent-specific color
        agent_colors = {
            AgentType.QUEEN: "ansiyellow",
            AgentType.WORKER: "ansicyan",
            AgentType.DRONE: "ansigreen",
            AgentType.FORAGER: "ansimagenta",
            AgentType.MANTIS: "ansiyellow",
            AgentType.DETECTIVE: "ansired",
        }

        color = agent_colors.get(message.agent_type, "ansiblack")

        # Use prompt_toolkit's FormattedText directly for simpler formatting
        return [
            (f"fg:{color}", message.sender),
            ("", ": "),
            ("", message.content),
        ]
    ...
    def add_message(
        ...
        # Detect code blocks if not explicitly set
        if not is_code and "```" in content:
            is_code = True
            # Extract code language from ```lang format
            import re
