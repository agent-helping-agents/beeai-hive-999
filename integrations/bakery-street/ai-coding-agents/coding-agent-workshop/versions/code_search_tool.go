...
func main() {
	...
	fmt.Println("🤖 Coding Agent Workshop - Code Search Tool")
	fmt.Println("===========================================")
	fmt.Println("Claude can now read files, list directories, run commands, edit files, AND search code!")
	...
	for {
		...
		// Send message to Claude with all capabilities including code search
	...
	req := openai.ChatCompletionRequest{
		...
		Tools: []openai.Tool{
			...
			},
			{
				...
				Function: openai.Function{
					Name:        "search_code",
					Description: "Search for patterns in code using ripgrep. Supports regex patterns and file type filtering.",
					Parameters: map[string]interface{}{
						"type": "object",
						"properties": map[string]interface{}{
							...
							"file_pattern": map[string]interface{}{
								"type":        "string",
								"description": "File pattern to search in (e.g., '*.go', '*.js', '*.py')",
							},
	...
	if len(choice.Message.ToolCalls) > 0 {
		...
		for _, toolCall := range choice.Message.ToolCalls {
			...
			case "search_code":
				...
				if verbose {
					fmt.Printf("🔍 Searching code: pattern='%s', file_pattern='%s', case_sensitive=%v\n", 
						args.Pattern, args.FilePattern, args.CaseSensitive)
				}

				// Search the code
				result, err := searchCode(args.Pattern, args.FilePattern, args.CaseSensitive)
				if err != nil {
					toolResults = append(toolResults, openai.ChatCompletionMessage{
						Role:       openai.ChatCompletionMessageRoleTool,
						Content:    fmt.Sprintf("Error searching code: %v", err),
						ToolCallID: toolCall.ID,
					})
				} else {
					toolResults = append(toolResults, openai.ChatCompletionMessage{
...
func searchCode(pattern, filePattern string, caseSensitive bool) (string, error) {
	// Check if ripgrep is available
	_, err := exec.LookPath("rg")
	if err != nil {
		// Fallback to grep if ripgrep is not available
		return searchWithGrep(pattern, filePattern, caseSensitive)
	}

	// Build ripgrep command
	args := []string{"--color=never", "--line-number", "--with-filename"}
	...
	if err != nil {
		// If ripgrep fails, try with grep as fallback
		return searchWithGrep(pattern, filePattern, caseSensitive)
	}
...
func searchWithGrep(pattern, filePattern string, caseSensitive bool) (string, error) {
	// Build grep command
	args := []string{"-n", "-r"}
	...
	if err != nil {
		// Check if it's just "no matches found" error
