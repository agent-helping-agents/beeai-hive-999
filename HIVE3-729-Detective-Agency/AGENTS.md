...
### 2.3 Linting
```bash
# Pylint / ruff (project uses ruff if present)
ruff check .
# Or use flake8
flake8 .
```
...
```bash
pytest tests/test_example.py::test_my_feature
```
...
- **Google‑style docstrings** for public call.
...
2. **Run** `ruff check . && mypy . && pytest` locally.
